...
export const ProjectEditPage: React.FC = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: EditProjectFormValues) => pqmService.updateProject(id, data as Partial<PQMProject>),
    onSuccess: () => {
      toast.success('Project updated successfully!');
      queryClient.invalidateQueries({ queryKey: ['pqm-projects'] });
      queryClient.invalidateQueries({ queryKey: ['pqm-project', id] });
  ...
  const { register, handleSubmit, formState: { errors, isDirty }, watch, setValue, reset } = useForm<EditProjectFormValues>({
  ...
  useEffect(() => {
    if (project) {
      reset({
        ...
        expected_project_end_date: project.expected_project_end_date || '',
        project_head_id: project.project_head_id || '',
        quality_head_id: project.quality_head_id || '',
        construction_incharge_id: project.construction_incharge_id || '',
        quality_incharge_id: project.quality_incharge_id || '',
  ...
  if (isError || !project) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh]">
      ...
      <button onClick={() => navigate('/pqm/projects')} className="mt-4 text-violet-600 hover:underline">
        Return to Projects
      </button>
    </div>
  );
      ...
      <div className="mx-auto max-w-6xl p-4 lg:p-8 animate-fadeInUp">
        <div className="mb-6 lg:mb-8">
          <PageHeader
            ...
            breadcrumbs={[
              { label: 'PQM' },
              { label: 'Projects', href: '/pqm/projects' },
              { label: project.name, href: backUrl },
              { label: 'Edit' },
            ]}
          />
        </div>
              ...
              <SectionCard title="Audit Context" icon={FolderGit2} iconColor="text-indigo-500" animDelay="delay-300">
                <div>
                  ...
                  <input
                    {...register('reason')}
                    type="text"
                    placeholder="e.g., Adjusting timeline..."
