...
import {
  User, ShieldAlert, Save, ArrowLeft, Loader2, Info, Eye, Type, Globe, Mail, Phone, Lock, Hash, Edit2, MessageSquare
} from 'lucide-react';

import { useUser } from '@/features/iam/users/hooks/useUsers';
import { useUpdateUser } from '@/features/iam/users/hooks/useUserMutations';
import { userUpdateSchema, UserUpdatePayload } from '@/features/iam/users/types/userTypes';
...
export const UserEditPage: React.FC = () => {
  ...
  const backUrl = `/platform/user-management/${id}`;

  const { register, handleSubmit, formState: { errors, isDirty, isSubmitting }, watch, control, reset, setValue } = useForm<UserUpdatePayload>({
  ...
  const isActiveWatch = watch('is_active') ?? true;
  const isSuperAdminWatch = watch('is_super_admin') ?? false;
  ...
  }, [isDirty, navigate, handleSubmit, backUrl]);
  ...
  const onSubmit = async (data: UserUpdatePayload) => {
    try {
      ...
      toast.error(err?.response?.data?.detail || 'An error occurred while updating the user.');
    }
  };
  ...
  if (isError || !user) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center">
        ...
        <p className="text-gray-500 mt-2 mb-6">This user does not exist or you lack permission to edit.</p>
        <button onClick={() => navigate('/platform/user-management')} className="text-violet-600 font-semibold hover:underline">
          &larr; Back to Users
        </button>
      </div>
    );
      ...
      <div className="flex h-full flex-col bg-gray-50/50 dark:bg-[#0a0a0a] animate-fadeInUp">
        <PageHeader
          ...
          breadcrumbs={[
            ...
            { label: 'Users', href: '/platform/user-management' },
            { label: user.username, href: backUrl },
            { label: 'Edit User' }
          ]}
          actions={
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      {isChangingPassword ? (
                        <div className="animate-fadeInDown mt-2">
                          <div className="relative">
                            ...
                            <input 
                              ...
                              autoFocus
                              className={`block w-full rounded-xl border ${errors.password ? 'border-rose-500 ring-rose-500' : 'border-gray-200 dark:border-gray-700 focus:border-violet-500 focus:ring-violet-500'} bg-gray-50/50 dark:bg-gray-800/50 py-2.5 pl-10 pr-4 text-sm dark:text-white transition-all`} 
                          ...
                          <div className="flex items-center justify-between mt-1">
                            ...
                            <button
                              ...
                              className="text-[10px] font-bold text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 uppercase tracking-wider"
                            >
                              Cancel
                            </button>
                          </div>
                        ...
                        <div className="rounded-xl border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 px-4 py-3 text-sm text-gray-500 dark:text-gray-400 italic">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <textarea
                          {...register('reason')}
                          rows={2}
                          placeholder="e.g. Updating contact info, changing roles"
        ...
        <div className="border-t border-gray-200 dark:border-gray-800 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl px-6 py-4 flex items-center justify-between animate-fadeInUp z-40">
          <p className="text-sm text-gray-500 font-medium">Any changes made will take effect immediately for the user.</p>
          <div className="flex items-center gap-3">
            <button
              onClick={() => isDirty ? setShowExitPrompt(true) : navigate(backUrl)}
