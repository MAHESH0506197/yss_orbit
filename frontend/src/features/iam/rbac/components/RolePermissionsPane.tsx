...
import { Key, Save, X, Check, AlertCircle, Search, Filter } from 'lucide-react';
import toast from 'react-hot-toast';

import { Role } from '../types/roleTypes';
import { useUpdateRole, usePermissions, useModules, useSubModules } from '../hooks/useRoles';

interface RolePermissionsPaneProps {
  role: Role;
  onCancel: () => void;
}

export function RolePermissionsPane({ role, onCancel }: RolePermissionsPaneProps) {
  ...
  // Find the role's parent module
  const roleParentModuleCode = useMemo(() => {
    if (!role.module_code) return null;
    const currentSub = subModules.find((s: any) => s.code === role.module_code);
    return currentSub?.parent_module_code || null;
  }, [role.module_code, subModules]);
  ...
  const groupedPermissions = useMemo(() => {
    ...
    permissions.forEach(p => {
      ...
      // Filter logic:
      if (!showAllModules && roleParentModuleCode) {
        if (mod !== roleParentModuleCode) {
          const pSub = subModules.find((s: any) => s.code === mod);
          if (!pSub || pSub.parent_module_code !== roleParentModuleCode) {
    ...
    return groups;
  }, [permissions, showAllModules, roleParentModuleCode, subModules]);
  ...
  const togglePermission = (permId: string) => {
    ...
    setSelectedPermissions(current => {
      if (current.includes(permId)) {
        return current.filter(id => id !== permId);
      }
      return [...current, permId];
    });
  };
  ...
  return (
    <div className="flex flex-col bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-xl overflow-hidden h-[800px] max-h-[85vh]">
      {/* Header */}
      <div className="flex-shrink-0 flex items-center justify-between border-b border-gray-200 dark:border-gray-800 px-6 py-4 bg-gray-50/50 dark:bg-gray-800/20 z-10 relative">
        <div>
          <h2 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <Key className="h-5 w-5 text-indigo-500" />
            Assign Permissions: <span className="text-indigo-600 dark:text-indigo-400">{role.name}</span>
          </h2>
          {isSystemRole ? (
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Select the modules on the left and assign specific access levels on the right.
            </p>
          )}
        </div>
        <div className="flex items-center gap-6">
          ...
          <button
            type="button"
            onClick={onCancel}
            className="rounded-full p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-gray-800 dark:hover:text-gray-300 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
            ...
            <>
              ...
              <div className="flex-shrink-0 px-8 py-5 border-b border-gray-100 dark:border-gray-800 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm sticky top-0 z-20 flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white capitalize flex items-center gap-3">
                      ...
                      <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400">
                        {activePermissions.filter(p => selectedPermissions.includes(p.id)).length} selected
                      </span>
                    </h3>
                  </div>
                  ...
                  {!isSystemRole && (
                    <button
                      onClick={() => toggleModulePermissions(activeModule)}
                    ...
                    >
                      {activePermissions.every(p => selectedPermissions.includes(p.id)) ? (
                        <>Unselect All</>
                      ) : (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-10">
                    {filteredPermissions.map((perm: any) => {
                      ...
                      return (
                        <label 
                          key={perm.id} 
                          className={`group flex items-start gap-4 p-5 rounded-2xl border-2 transition-all duration-300 ${!isSystemRole ? 'cursor-pointer hover:-translate-y-1 hover:shadow-xl' : 'opacity-80 cursor-not-allowed'} ${isSelected ? 'border-indigo-500 bg-indigo-50/80 shadow-md shadow-indigo-500/10 dark:border-indigo-500/60 dark:bg-indigo-500/10' : 'border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm hover:border-indigo-200 dark:hover:border-indigo-800/50 hover:bg-gray-50 dark:hover:bg-gray-800'}`}
                        >
      ...
      {!isSystemRole && (
        <div className="flex-shrink-0 border-t border-gray-200 dark:border-gray-800 px-8 py-5 bg-gray-50/80 dark:bg-gray-900/80 backdrop-blur-sm flex items-center justify-between z-10">
          <div className="text-sm font-semibold text-gray-600 dark:text-gray-400">
            Total assigned: <span className="text-indigo-600 dark:text-indigo-400 text-base">{selectedPermissions.length}</span>
          </div>
          <div className="flex items-center gap-4">
            <button
              type="button"
              onClick={onCancel}
              className="px-6 py-2.5 text-sm font-bold text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
            >
              Cancel
            </button>
            <button
              ...
              className="inline-flex items-center justify-center gap-2 px-8 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-indigo-600 to-violet-600 rounded-xl hover:shadow-lg hover:shadow-indigo-500/30 transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500/50 disabled:opacity-70"
            >
