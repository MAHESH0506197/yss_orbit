/**
 * YSS Orbit - ResetPasswordPage
 * Submits new password via POST /api/v1/auth/password/reset/
 * Expects username and otp from location.state
 */
...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) { setError(data?.error?.message || data?.detail || data?.new_password?.[0] || 'Reset failed. The OTP may be invalid or expired.'); return; }
      setDone(true);
  ...
  return (
    <AuthLayout heading="Set new password" subheading="Choose a strong password for your account" leftPanelContent={leftPanelContent}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        {done ? (
          <div style={{ background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.25)', borderRadius: 12, padding: '20px 18px', textAlign: 'center' }}>
            ...
            <div style={{ fontWeight: 700, marginBottom: 6, color: isDark ? '#ffffff' : '#0f172a' }}>Password updated!</div>
            <div style={{ fontSize: 13, color: isDark ? 'rgba(255,255,255,0.5)' : 'rgba(15,23,42,0.5)' }}>Redirecting you to sign in…</div>
          </div>
        ) : (
          <>
            ...
            <form onSubmit={handleSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                <label htmlFor="newpass" style={{ fontSize: '0.85rem', fontWeight: 500, color: isDark ? 'rgba(255,255,255,0.85)' : 'rgba(15,23,42,0.85)' }}>
                  New Password
                </label>
                <div style={{ position: 'relative' }}>
                  <input 
                    ...
                    disabled={loading} 
                    style={{
                      width: '100%', height: '2.75rem', padding: '0 2.5rem 0 0.875rem', borderRadius: '0.625rem',
                    ...
                    onBlur={e => { e.currentTarget.style.borderColor = isDark ? 'rgba(255,255,255,0.12)' : 'rgba(99,102,241,0.22)'; e.currentTarget.style.boxShadow = 'none'; }}
                  />
                  <button type="button" onClick={() => setShowPass(s => !s)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: isDark ? 'rgba(255,255,255,0.4)' : 'rgba(15,23,42,0.4)', padding: 0, fontSize: 14 }}>
                    {showPass ? 'ðŸ™ˆ' : 'ðŸ‘ï¸'}
                  </button>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                <label htmlFor="confirmpass" style={{ fontSize: '0.85rem', fontWeight: 500, color: isDark ? 'rgba(255,255,255,0.85)' : 'rgba(15,23,42,0.85)' }}>
                  Confirm Password
                </label>
                <input 
                  id="confirmpass" 
