# yss_orbit\backend\rewrite_logic_v2.py
import os
import re
...
def get_model_name_from_file(filename):
    # e.g., organization_view.py -> Organization
    name = filename.replace("_view.py", "").replace("_service.py", "").replace("_serializer.py", "")
    ...
    if os.path.exists(views_dir):
        for view_file in os.listdir(views_dir):
            if view_file.endswith(".py") and not view_file.startswith("__"):
                ...
                # We skip heavily customized files like auth_views.py to avoid breaking them
...
class {model_name}Service:
    ...
    def process(cls, data: Dict[str, Any], user_id: str = "system") -> bool:
        ...
        return True
'''
                # don't overwrite user_service.py entirely if it's already complex, but for this boilerplate, it's fine.
...
from apps.{app}.models.{serializer_file.replace("_serializer.py", "_model")} import {model_name}
