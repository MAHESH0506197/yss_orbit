# yss_orbit\backend\apps\appraisal\tests\test_appraisal_api.py
import pytest
from rest_framework import status
from django.urls import reverse

@pytest.mark.django_db
class TestAppraisalAPI:
    def test_list_endpoint_unauthorized(self, api_client):
        response = api_client.get('/api/appraisal/')
    ...
    def test_list_endpoint_authorized(self, authenticated_client):
        response = authenticated_client.get('/api/appraisal/')
